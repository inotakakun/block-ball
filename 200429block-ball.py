# ブロック崩し
from tkinter import *
import random,platform,winsound
...
# ゲーム中で使う変数の一覧
blocks = []
block_size = {"x": 40, "y": 30}
ball = {"dirx": 15, "diry": -15, "x": 350, "y": 300, "w": 20}
bar = {"x": 100, "w": 400}
is_gameover = False
point = 0
...
def draw_objects():
    ...
    cv.create_oval(ball["x"] - ball["w"], ball["y"] - ball["w"],
        ball["x"] + ball["w"], ball["y"] + ball["w"], fill="white")
    # バーを描画
    cv.create_rectangle(bar["x"]-(bar["w"]/2), 390, bar["x"] + bar["w"]-(bar["w"]/2), 400, 
        fill="yellow")
...
def move_ball():
    ...
    if by < 0:
        ball["diry"] *= -1
    # プレイヤーの操作するバーに当たった？
    if by > 390 and (bar["x"]-(bar["w"]/2) <= bx <= (bar["x"] + bar["w"]-(bar["w"]/2))):
        ball["diry"] *= -1
        beep(2000, 100)
